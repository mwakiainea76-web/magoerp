...
import { ArrowLeft, BadgeCheck, Coins, Pencil, X } from "lucide-react";
...
export function FeeTemplateAssignmentsPage() {
  ...
  async function submit(values) {
    ...
    try {
      ...
      reset({ assignment_scope: "course", issuance_type: "per_year", course_curriculum_id: "", department_id: "", academic_year_id: "", session_number: "", year_level: "", is_approved: true });
  ...
  async function toggleApproval(assignment) {
    const nextApproved = !assignment.is_approved;
    setActivationId(assignment.id);
    try {
      await assignmentsApi.update(templateId, assignment.id, { is_approved: nextApproved });
      toast.success(nextApproved ? "Fee assignment activated." : "Fee assignment deactivated.");
      await load();
    } catch (error) {
  ...
  return (
    <section className="space-y-5">
      <div className="flex items-start justify-between gap-4">
        ...
        <Link to="/finance/fee-structures" className="inline-flex items-center gap-2 text-sm font-medium text-slate-500 hover:text-slate-900"><ArrowLeft className="size-4" />Back</Link>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <form onSubmit={handleSubmit(submit)} className="space-y-4">
          ...
          {issuanceType === "per_year" ? <div className="rounded-xl border border-sky-200 bg-sky-50 p-4"><p className="text-sm font-medium text-sky-900">Yearly split ratios</p><p className="mt-1 text-xs text-sky-700">One portion is stored for every progression session. All portions can be used by active intakes.</p>{yearSessions.length ? <><div className="mt-3 grid gap-3 sm:grid-cols-3">{yearSessions.map((session, index) => <div key={session.id}><p className="text-xs text-slate-600">{session.name}</p><FormInput type="number" label="Ratio (%)" min="0.01" max="100" step="0.01" value={ratios[index] ?? ""} onChange={(event) => setRatios((current) => current.map((ratio, ratioIndex) => ratioIndex === index ? event.target.value : ratio))} /></div>)}</div><p className="mt-2 text-xs font-medium text-sky-800">Total: {ratios.reduce((sum, ratio) => sum + Number(ratio || 0), 0).toFixed(2)}%</p></> : <p className="mt-3 text-sm text-sky-700">Select an academic year to configure its session ratios.</p>}</div> : null}
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-5 py-4"><div><h2 className="font-semibold text-slate-900">Existing Assignments</h2><p className="mt-0.5 text-xs text-slate-500">Search by course, curriculum, department name, or code.</p></div><form onSubmit={applySearch} className="flex w-full gap-2 sm:w-auto"><FormInput value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="Search course name..." className="min-w-0 flex-1 sm:w-72" rightIcon={searchInput ? <X className="size-4" /> : undefined} onRightIconClick={clearSearch} rightIconLabel="Clear search" /><button type="submit" className="h-9 rounded-lg bg-slate-700 px-4 text-sm font-medium text-white hover:bg-slate-800">Search</button></form></div>
        {loading ? <p className="p-5 text-sm text-slate-500">Loading...</p> : assignments.length === 0 ? <p className="p-5 text-sm text-slate-500">{query ? "No assignments match your search." : "No assignments yet."}</p> : <div className="divide-y divide-slate-100">{assignments.map((assignment) => <div key={assignment.id} className="p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-medium text-slate-900">{assignment.assignment_target_name} Ã‚Â· {assignment.year_level === 0 ? "All Years" : `Year ${assignment.year_level}`}</p><p className="mt-1 text-xs text-slate-500">{assignment.issuance_type === "per_year" ? "Academic year" : `Session ${assignment.session_number}`} Â· {money(assignment.split_amount)}</p></div><button type="button" onClick={() => toggleApproval(assignment)} className={`rounded-full px-3 py-1 text-xs font-semibold ${assignment.is_approved ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`} disabled={activationId === assignment.id}>{assignment.is_approved ? <span className="inline-flex items-center gap-1"><BadgeCheck className="size-3" />Active</span> : activationId === assignment.id ? "Activating..." : "Activate fee"}</button></div>{assignment.issuance_type === "per_year" ? <div className="mt-4 grid gap-3 md:grid-cols-3">{(assignment.child_assignments ?? []).map((portion) => <div key={portion.id} className="rounded-xl border border-slate-200 p-3"><div className="flex justify-between gap-2"><div><p className="text-sm font-medium text-slate-800">{portion.academic_session_name}</p><p className="text-xs text-slate-500">{portion.split_ratio}% Ã‚Â· {money(portion.split_amount)}</p></div>{portion.dormant ? <button type="button" onClick={() => editDormant(portion)} className="rounded-lg p-2 text-sky-600 hover:bg-sky-50" title="Edit dormant portion"><Pencil className="size-4" /></button> : null}</div><span className={`mt-2 inline-flex rounded-full px-2 py-0.5 text-[11px] font-semibold ${portion.dormant ? "bg-slate-100 text-slate-600" : "bg-emerald-50 text-emerald-700"}`}>{portion.dormant ? "Dormant" : "Active"}</span></div>)}</div> : null}</div>)}</div>}
