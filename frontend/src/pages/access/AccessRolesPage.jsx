...
export function AccessRolesPage() {
  ...
  }, [rolesApi, page, perPage, query, reloadKey, sortBy, sortDirection]);
  ...
  return (
    <section className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[18px] font-semibold tracking-[-0.01em] text-slate-950">Access Roles</h1>
          <p className="text-[13px] text-slate-500">Manage user roles and their permissions</p>
        </div>
      ...
      <Table>
        <TableHeader>
          <h2 className="flex items-center gap-2 text-[1.0625rem] font-semibold text-slate-900">
        ...
        {isLoading ? (
          <div className={`px-5 py-10 text-slate-500 ${bodyTextClassName}`}>Loading roles...</div>
        ) : roles.length === 0 ? (
          <div className={`px-5 py-10 text-slate-500 ${bodyTextClassName}`}>No roles found for the current filters.</div>
        ) : (
          <TableWrapper>
            <Thead>
              <tr>
                <Th className="w-10 text-center">#</Th>
                <SortableTh sortKey="name" sortBy={sortBy} sortDirection={sortDirection} onSort={handleSort}>Role Name</SortableTh>
                <SortableTh sortKey="guard_name" sortBy={sortBy} sortDirection={sortDirection} onSort={handleSort}>Guard</SortableTh>
                <Th>Permissions</Th>
        ...
        <TableFooter>
          <p className={`text-slate-500 ${bodyTextClassName}`}>
            {meta.total > 0 ? `Showing ${meta.from} to ${meta.to} of ${meta.total} roles` : "No results"}
          </p>
