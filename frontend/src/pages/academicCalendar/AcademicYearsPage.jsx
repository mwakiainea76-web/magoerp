...
export function AcademicYearsPage() {
  ...
  }, [yearsApi, page, perPage, query, reloadKey, sortBy, sortDirection]);

  async function handleDelete(year) {
    const confirmed = window.confirm(`Delete ${year.name}?`);
    if (!confirmed) return;
  ...
  return (
    <section className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[13px] text-slate-500">
            Manage academic calendar years and their sessions
          </p>
        </div>
      ...
      <Table>
        <TableHeader>
          <h2 className="text-[1.0625rem] font-semibold text-slate-900">Academic Years Directory</h2>
        </TableHeader>

        {isLoading ? (
          <div className={`px-5 py-10 text-slate-500 ${bodyTextClassName}`}>Loading academic years...</div>
        ) : years.length === 0 ? (
          <div className={`px-5 py-10 text-slate-500 ${bodyTextClassName}`}>No academic years found for the current filters.</div>
        ) : (
          <TableWrapper>
            <Thead>
              <tr>
            ...
            <Tbody>
              {years.map((year, index) => (
                <tr key={year.id}>
                  ...
                  <Td>{year.code}</Td>
                  <Td>{year.name}</Td>
                  <Td className="whitespace-nowrap">{year.start_date ?? "-"}</Td>
                  ...
                  <Td>
                    <span
                      className={`inline-flex items-center rounded-full px-2.5 py-1 text-[11px] font-semibold ${
                        year.is_active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"
                      }`}
                    >
        ...
        <TableFooter>
          <p className={`text-slate-500 ${bodyTextClassName}`}>
            {meta.total > 0 ? `Showing ${meta.from} to ${meta.to} of ${meta.total} years` : "No results"}
          </p>
